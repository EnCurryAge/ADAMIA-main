...
def data_aug_method(strength=0.1, mode='sw', combination='r+f+c+g'):
    ...
    if mode == 'W' and combination == 'R' and 0 <= strength <= 0.3:
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(strength, 1)),
                                              transforms.ToTensor()
                                              ])
        ...
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32),
                                              transforms.RandomGrayscale(p=1 - strength),
                                              transforms.ToTensor()
                                              ])
    ...
    # second under the strong setting, using two DA transformations
        ...
        data_transforms = transforms.Compose([transforms.RandomGrayscale(p=strength),
                                              transforms.RandomHorizontalFlip(p=1),
                                              transforms.ToTensor()
                                              ])
    elif mode == 'S' and combination == 'R+G' and 0.3 <= strength <= 0.6:
        color_jitter = transforms.ColorJitter(0.8 * strength, 0.8 * strength, 0.8 * strength, 0.2 * strength)
        data_transforms = transforms.Compose([transforms.RandomGrayscale(p=1),
                                              transforms.RandomApply([color_jitter], p=1),
                                              transforms.ToTensor()
                                              ])
    elif mode == 'S' and combination == 'H+C' and 0.3 <= strength <= 0.6:
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(0, strength)),
                                              transforms.RandomHorizontalFlip(p=1),
                                              transforms.ToTensor()
                                              ])
    ...
    # third use the aggressive DA, using third or more DA transformations
    elif mode == 'A' and combination == 'R+H+C' and 0 <= strength <= 1:
    ...
    elif mode == 'A' and combination == 'R+H+G' and 0 <= strength <= 1:
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(0, strength)),
                                              transforms.RandomHorizontalFlip(p=1),
                                              transforms.RandomGrayscale(p=1),
                                              transforms.ToTensor()
                                              ])
    elif mode == 'A' and combination == 'R+C+G' and 0 <= strength <= 1:
        color_jitter = transforms.ColorJitter(1.6 * strength, 1.6 * strength, 1.6 * strength, 0.4 * strength)
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(0, strength)),
                                              transforms.RandomApply([color_jitter], p=1),
                                              transforms.RandomGrayscale(p=1),
    ...
    elif mode == 'A' and combination == 'H+C+G' and 0 <= strength <= 1:
        color_jitter = transforms.ColorJitter(1.6 * strength, 1.6 * strength, 1.6 * strength, 0.4 * strength)
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(0, strength)),
                                              transforms.RandomHorizontalFlip(p=1),
                                              transforms.RandomApply([color_jitter], p=1),
    ...
    elif mode == 'A' and combination == 'R+H+C+G' and 0 <= strength <= 1:
        color_jitter = transforms.ColorJitter(1.6 * strength, 1.6 * strength, 1.6 * strength, 0.4 * strength)
        data_transforms = transforms.Compose([transforms.RandomResizedCrop(size=32, scale=(0, strength)),
                                              transforms.RandomHorizontalFlip(p=1),
                                              transforms.RandomApply([color_jitter], p=1),
...
def trans():
    # reminder here!
    # ppp -> float, mode -> sw / w / s, combination -> Resize(r) / Flip (f) / Color_jitter (c) / Grayscale (g)
